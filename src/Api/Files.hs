...
upload = do
  fs <- handleUploads
  envIO $ print fs
  status notModified304 -- TODO

filePath' :: String -> T.Text -> T.Text -> String
filePath' pfx hash name =
    intercalate "/" . (pfx:)
    $ map
      T.unpack
      [ T.take 2 hash,
...
propIsFileWithHash h property =
  ...
  S..&&
  ((property S.! #propertyType S..== S.literal File)
...
handleUploads = files >>= traverse handleUpload'
  where
    handleUpload' (_, x) =
      (Just <$> handleUpload x) `rescue` const (pure Nothing)
  ...
  case res of
    [d] -> do
      case filePropertyToDesc d of
        Just (h,_,n) -> fileUrl h n >>= redirect . T.pack
